...
# Configuration
# -------------------------------
TEMPLATE_PATH = "../Templates/Hindi/11-12_Hindi.docx"  # Word template
EXCEL_PATH = "../input.xlsx"   # Our master sheet
SHEET_NAME = "Master_Sheet"  # Our master sheet name
OUTPUT_DIR = "../Reports/hindi"             # Directory to save reports
GRAPH_PERSONALITY_DIR = "../graphs/class_11-12/personality"  # Directory for personality graphs
GRAPH_INTELLIGENCE_DIR = "../graphs/class_11-12/intelligence"  # Directory for intelligence graphs
...
for index, row in data.iterrows():
    ...
    # Only process students in class 11-12
    if row.get("Class") not in [11, 12]:
        print(f"Skipping {student_name} - not in class 11-12")
        continue
    ...
    try:
        ...
        print(f"Palak has a crush on before rendering")
        ...
        print(f"Palak has a crush on parth")
        # Get organized path (School > Class > Section)
        if school or class_val or section:
            # Use organized structure
            report_dir = get_report_path(school, class_val, section, base_dir="../Reports/hindi")
            print(f"Using organized path: {report_dir} for {student_name}")
            logging.info(f"Using organized path: {report_dir} for {student_name}")
        else:
            # Fall back to default directory
            report_dir = OUTPUT_DIR
            print(f"Palak has a crush on atul")
        # Build DOCX filename
