...
# Configuration
# -------------------------------
TEMPLATE_PATH = "../Templates/9-10.docx"  # Word template
...
GRAPH_PERSONALITY_DIR = "../graphs/class_9-10/personality"  # Directory for personality graphs
GRAPH_INTELLIGENCE_DIR = "../graphs/class_9-10/intelligence"  # Directory for intelligence graphs
...
for index, row in data.iterrows():
    ...
    # Only process students in class 9-10
    if row.get("Class") not in [9, 10]:
        print(f"Skipping {student_name} - not in class 9-10")
        continue
    ...
    try:
        ...
        if not ai_summary_val or not ls_summary_val:
            logging.warning(f"Skipping {student_name}: missing AI summaries (AI_Summary/Learning_Style_Summary)")
            continue
        ...
        # Check for Naturalistic (try with and without trailing space)
        ...
        # Only add Naturalistic if score > 0
        ...
        alias_map = {
            "Logical-Mathematical": ["Logical-Mathematical", "Logical", "Logical Reasoning", "Logical reasoning", "logical-mathematical", "logical"],
        ...
        def get_score(canonical_label: str):
            # Try all aliases, including versions with trailing spaces
            aliases = alias_map.get(canonical_label, [canonical_label])
            # Also try with trailing space
            aliases_with_spaces = [a + " " for a in aliases]
            all_keys_to_try = aliases + aliases_with_spaces
        ...
        if school or class_val or section:
            ...
            report_dir = get_report_path(school, class_val, section, base_dir="../Reports/english")
